...
class downloadInit { 
	...
	private $allow = array(".jpg",".jpeg",".bmp",".png", ".gif", ".png", ".rar",".zip",".7z",".txt",".docx",".xlsx",".xls",".pptx",".ppt",".pdf",'.docx'); //允许下载的文件类型
	 
	/**
	 *	文件下载
	 * 	@param  string  $file_name    文件名
	 * 	@param  string  $server_path  文件目录
	 *  @param  string  $show_name    下载文件显示的名字，如 将 abc.png 下载显示为：图片.png
	 * 	@param  string  $mime_type    传输类型
	 *  @return 
	 */
	public function down($file_name, $server_path = './',$show_name='', $mime_type = 'application/octet-stream') {
		...
		if(!empty($show_name) && strlen($show_name) > 1){
		    $file_name = $show_name;
		}
